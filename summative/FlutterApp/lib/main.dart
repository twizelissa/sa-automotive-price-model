...
class _CarPredictionPageState extends State<CarPredictionPage> {
  ...
  // API INTEGRATION: POST request to FastAPI endpoint
  Future<void> _predictPrice() async {
    ...
    try {
      ...
      ).timeout(const Duration(seconds: 30)); // Handle network timeouts
      ...
      setState(() {
        if (e.toString().contains('TimeoutException')) {
          _errorMessage = 'Request timeout. Please check your connection and try again.';
        } else {
          _errorMessage = 'Network error: Unable to connect to server. Please try again.';
        }
        _isLoading = false;
      });
    }
            ...
            children: [
              ...
              DropdownButtonFormField<int>(
                ...
                onChanged: (int? value) {
                  setState(() {
                    _isLuxury = value ?? 0;
                    _luxuryDisplayValue = value == 1 ? 'Yes' : 'No';
                  });
                },
              ),
              const SizedBox(height: 30),
